    ...
    if (FIREBASE_AUTH.currentUser) {
      try {
        ...
        const [videoDownloadUrl, thumbnailDownloadUrl] = await Promise.all([
          saveMediaToStorage(
            video,
            `post/${FIREBASE_AUTH.currentUser.id}/${storagePostId}/video`,
          ),
          saveMediaToStorage(
            thumbnail,
            `post/${FIREBASE_AUTH.currentUser.id}/${storagePostId}/thumbnail`,
          ),
        ]);

        await addDoc(collection(FIREBASE_DB, "post"), {
          creator: FIREBASE_AUTH.currentUser.id,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createPost.rejected, (state, action) => {
        state.loading = false;
        state.error = action.error.content || null;
      })
